...
double input = 80, output = 50, setpoint = 2000;
...
boolean tuning = false; //------------------- changed this was originally false
...
void loop()
{
    ...
    if (tuning)
    {
        ...
        byte val = (aTune.Runtime()); //doesn't seem to trigger
        //Serial.print("val = ");
        //Serial.println(val);
        if (val != 0)
        {
        ...
        //{ //we're done, set the tuning parameters
            //Serial.println("Finished Tuning");
            kp = aTune.GetKp();
            ki = aTune.GetKi();
            kd = aTune.GetKd();
            Serial.println(kp);
